...
```{r run-minpatch}
cat("Running MinPatch (this may take several minutes)...\n")
...
cat("MinPatch completed in", round(processing_time, 2), "minutes\n")
...
```{r feature-analysis}
# Analyze how well conservation targets are met
feature_rep <- calculate_feature_representation(
  result$minpatch_data$unit_dict,
  result$minpatch_data$abundance_matrix,
  result$minpatch_data$target_dict
)

# Summary statistics
targets_met <- sum(feature_rep$target_met)
mean_achievement <- mean(feature_rep$proportion_met, na.rm = TRUE)
...
worst_features <- feature_rep[order(feature_rep$proportion_met), ][1:5, ]
cat("\nFeatures with lowest target achievement:\n")
print(worst_features[, c("feature_id", "proportion_met", "target_met")])
```
...
gg1 <- ggplot(data = dat) +
  ...
  labs(title = "Original prioritizr Solution") +
  theme_void()
...
gg3 <- ggplot(data = dat) +
  ...
  scale_fill_manual(
    values =  c(
      "No Change" = "lightgray",
      "Added by MinPatch" = "darkred",
      "Removed by MinPatch" = "darkblue",
      "Retained" = "darkgreen"),
    name = "Changes"
  ) +
  labs(title = "Changes Made by MinPatch") +
  theme_void()

patchwork::wrap_plots(gg1, gg2, gg3, ncol = 2)
...
# Compare scenarios
comparison_conservative <- compare_solutions(original_solution, result_conservative)
```
