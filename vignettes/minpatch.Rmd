...
## Enhanced prioritizr Integration

MinPatch now leverages prioritizr's summary functions to reduce code duplication and ensure consistency:

- **Cost calculations**: Uses `prioritizr::eval_cost_summary()` and `prioritizr::eval_boundary_summary()`
- **Feature representation**: Uses `prioritizr::eval_feature_representation_summary()` and `prioritizr::eval_target_coverage_summary()`
- **Planning unit counts**: Uses `prioritizr::eval_n_summary()`

This integration provides more accurate and consistent results while reducing the maintenance burden of duplicate functionality.
...
# Solve the problem
cat("Solving prioritizr problem...\n")
s <- solve(p)
```
...
ggplot(data = s) + 
  geom_sf(aes(fill = as.logical(solution_1)))
...
#TODO Lets clean up the output here. I think result should be a list with the output
# sf dataframe as the first object and then a 2nd object with all the debugging stuff. 
# To the first dataframe, I want to add the sf coords, the original solution, what 
# patches were identified, which ones were added, and then which ones were whittled. 
...
```{r feature-analysis}
# Analyze feature representation using prioritizr functions directly
...
```{r visualization}
visualize_minpatch_results(result, title = "MinPatch Results")
...
```{r multiple-scenarios, eval=FALSE}
...
# Compare scenarios
comparison_conservative <- compare_solutions(result_conservative)
```
