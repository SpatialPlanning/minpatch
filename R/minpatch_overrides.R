...
simulated_whittling <- function(minpatch_data, verbose = TRUE) {
  ...
  # --- DEBUG SETTINGS (MESSAGES ONLY) ---
  debug_first_n <- 25       # detailed candidate-level messages for first N iterations
  progress_every <- 100     # keep long runs readable
  # -------------------------------------

  if (verbose) {
    cat("Stage 3 (Simulated whittling) diagnostics:\n")
    cat("  boundary_penalty =", boundary_penalty, "\n")
    cat("  min_patch_size   =", min_patch_size, "\n")
    cat("  debug_first_n    =", debug_first_n, "\n")
  }
  ...
  while (iteration < max_iterations) {
    ...
    if (verbose && iteration %% progress_every == 0) {
      cat("  Whittling iteration", iteration,
          "| keystone =", length(keystone_pu_id_set), "\n")
    }
    ...
    if (verbose && iteration <= debug_first_n) {
      cat("\n    [candidate]", candidate_unit,
          "| whittle_score =", whittle_scores[[candidate_unit]],
    ...
    removal_result <- can_remove_unit(candidate_unit, minpatch_data, feature_amounts,
                                      verbose = (verbose && iteration <= debug_first_n))
    ...
    # IMPORTANT: keep local unit_dict + minpatch_data consistent for the next loop
...
find_edge_units <- function(minpatch_data) {
  ...
  for (unit_id in names(unit_dict)) {
    if (unit_dict[[unit_id]]$status %in% c(1, 2)) {  # Consider both selected (1) and conserved (2) units
      ...
      neighbor_indices <- which(boundary_matrix[unit_idx, , drop = FALSE] > 0)
...
#' @param minpatch_data List containing all MinPatch data structures
#' @param feature_amounts Optional pre-computed feature amounts
#' @param verbose Logical, whether to print per-check messages
#'
#' @return Logical indicating if unit can be removed
#' @keywords internal
can_remove_unit <- function(unit_id, minpatch_data, feature_amounts = NULL, verbose = FALSE) {

  unit_dict <- minpatch_data$unit_dict

  # Don't remove conserved units (status 2)
  if (unit_dict[[unit_id]]$status == 2) {
    if (verbose) cat("      FAIL: conserved (status=2)\n")
    return(FALSE)
  }
  ...
  if (removal_violates_targets(unit_id, minpatch_data, feature_amounts)) {
    if (verbose) cat("      FAIL: targets (would drop below target)\n")
    return(FALSE)
  }

  # Check if removal would make patch too small
  if (removal_makes_patch_too_small(unit_id, minpatch_data)) {
    if (verbose) cat("      FAIL: min_patch_size\n")
    return(FALSE)
  }

  # Check if removal would increase cost (this is where boundary_penalty shows up)
  if (removal_increases_cost(unit_id, minpatch_data, verbose = verbose)) {
    if (verbose) cat("      FAIL: boundary_penalty (removal increases cost)\n")
    return(FALSE)
  }

  # Check if removal would split patch into non-viable pieces
  if (removal_splits_patch_nonviably(unit_id, minpatch_data)) {
    if (verbose) cat("      FAIL: split (creates non-viable patch)\n")
    return(FALSE)
  }

  if (verbose) cat("      PASS: removable\n")
  return(TRUE)
}
...
#' @param unit_id ID of unit to potentially remove
#' @param minpatch_data List containing all MinPatch data structures
#' @param verbose Logical, whether to print boundary penalty calculation
#'
#' @return Logical indicating if removal would increase cost
#' @keywords internal
removal_increases_cost <- function(unit_id, minpatch_data, verbose = FALSE) {
  ...
  if (boundary_penalty == 0) {
    if (verbose) cat("      [boundary] penalty=0 -> skip\n")
  ...
  if (verbose) {
    cat("      [boundary] unit_cost =", unit_cost,
        "| ΔB =", boundary_change,
        "| λ =", boundary_penalty,
        "| λΔB =", boundary_cost_change,
        "| λΔB - cost =", total_cost_change, "\n")
  }
